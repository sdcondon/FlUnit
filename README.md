...
- Specific, highest-priority first:
  - *(Jan)* Add in initial test settings - initial settings likely to include allowing specification of strategy for result naming and duration records (both of which currently make some "sensible" decisions which may not be appropriate in all situations). Initial factoring of core test discovery and execution logic away from VSTest-specific classes may happen as part of this. I'm wary of needless complexity until such time as a second adapter exists - so likely to stop short of fullly-developed abstractions for test platform. We'll see..
